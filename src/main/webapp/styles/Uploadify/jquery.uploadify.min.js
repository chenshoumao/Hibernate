...
SWFUpload.Console.writeLine = function(g) {
    ...
    var a = {
        onDialogOpen: function() {
            var d = this.settings;
            this.queueData.errorMsg = "文件不能够上传:";
        ...
        onSelect: function(h) {
            ...
            for (var g in this.queueData.files) {
                ...
                if (f.uploaded != true && f.name == h.name) {
                    var e = confirm('文件 "' + h.name + '" 已存在于上传队列中.\n您想替换掉服务器端的文件吗?');
                    if (!e) {
            ...
            if (c.inArray("onSelectError", e.overrideEvents) < 0) {
                ...
                case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                    if (e.queueSizeLimit > f) {
                        this.queueData.errorMsg += "\n选择的文件的数量超过了剩余的上传限制  " + f + "个.";
                    } else {
                        this.queueData.errorMsg += "\n选择的文件的数量超过了剩余的上传限制 (" + e.queueSizeLimit + ").";
                    }
                    break;
                case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                    this.queueData.errorMsg += '\n文件 "' + d.name + '" 超过单文件限制 (' + e.fileSizeLimit + "KB).";
                    break;
