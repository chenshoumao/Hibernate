$(function() {
	...
	$("#btnSave").click(function(evt) {
		...
		for (var i=0; i<gatewayCount; i++) {
			...
			$("#conditionGrid_"+i+" tbody tr").each(function(index) {
				...
				var condGrpName=$(tdcells[7]).text();
				var rowCond={};
				...
				rowCond.condGrpName=condGrpName;
	...
	$(":text[id^='condGrpName_']").bind("keyup paste", function(evt) {
		var value=$(this).val();
...
function openConditionWnd(index, sender) {
	...
	if (sender) {
		...
		$("#condGrpName_"+index).val(condGrpName);
	} else {
		//新建初始化控件
...
function saveConditionRow(index, sender) {
	...
	var condGrpName=$("#condGrpName_"+index).val();
	if (!sender) {
		//新建
