...
//同步流程图信息
function getVisualProcessModel() {
	var rows = $('#datagrid').datagrid('getChecked');
	if (rows.length > 0) {
		$.messager.confirm('温馨提示', '同步快照流程图将会覆盖更新快照流程图信息,是否继续？', function(r) {
			if (r) {
				//开启遮罩层
				$.messager.progress({ 
					title: '正在同步...', 
				...
				$.ajax({
					url : $("#basePath").val()
							+ "/console/bpm/processSnapshot/getVisualProcessModel.action",
					type : "post",
					data : jsonData,
					dataType : "json",
					success : function(result) {
...
function openProcessModel() {
	...
	if (rows.length > 0) {
		for (var i = 0; i < rows.length; i++) {
			var url = $("#basePath").val()
			+ "/console/diagram/diagramManage.action?snapshotId="+rows[i].snapshotId;
			window.open(url,rows[i].snapshotName);
		}
	}
}
