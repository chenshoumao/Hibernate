/**
 * 这个js与systemMeta.js的功能基本一致，只是多了权限的控制
 */
...
function openOwnerDlg() {
	...
	$.ajax({
		url : $("#basePath").val()
				+ "/console/systemMeta/getOwnersData.action",
		type : "post",
		data : {
...
function createOrUpdateOwner() {
	...
	url=$("#basePath").val()+ "/console/systemMeta/createOrUpdateOwner.action?metaCataId="+$("#metaCataId").val();
...
function datagrid(metaName) {
	$('#datagrid').datagrid({
		height : '95%',
		fit : true,
		url : $("#basePath").val() + '/console/systemMeta/ownerFindByPage.action',
		method : 'POST',
		queryParams : {
