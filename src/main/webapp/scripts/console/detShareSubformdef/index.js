$(function() {
	...
	var nspath=ctxPath+"/console/detShareSubform";  //命名空间路径
    var datAppId=$("#datAppId").val();

    $("#grid1").datagrid({
		url:nspath+"/shows.action?r1="+Math.random(),
		toolbar:"#toolbar",
		pagination:"true",
		pageSize:"50",
		rownumbers:"true",
		...
		onDblClickRow: function (rowIndex, rowData) {
			...
			$("#iframe1").attr("src", nspath+"/modify/"+rowData.subformId+".xsp?r1="+Math.random());
	...
	$("#btnnew").click(function(evt) {
		var appid=$("#datAppId").val();
		$("#iframe1").attr("src", nspath+"/create.xsp?r1="+Math.random());
	...
	$("#btndel").click(function(evt) {
		...
		if (selRows.length>0) {
			...
			$.messager.confirm('提示', "是否删除选中的 "+selRows.length+" 个子表单？", function(r){
				if (r){
					$.post(nspath+"/deleteform.action", {formIds:formIds}, function(data) {
						if (data.success) {
							alert("删除子表单成功！");
