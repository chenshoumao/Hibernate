...
function ajaxLoading(){ 
	...
	$("<div class=\"datagrid-mask-msg\"></div>").html("数据加载中请稍候。。。").appendTo("body").css({display:"block",left:($(document.body).outerWidth(true) - 190) / 2,top:($(window).height() - 45) / 2}); 
	}
...
$(function() {
	...
	$('.cs-navi-tab').click(function() {
		...
		if(target=== undefined){
		var title = $(this).parent().attr("name")+"-"+$this.text();
		addTab(title, href);
